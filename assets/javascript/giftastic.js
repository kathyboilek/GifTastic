...
document.addEventListener('DOMContentLoaded', () => {
  // create an array of strings
	var topics = ['betty davis', 'frank sinatra', 'alfred hitchcock', 'film noir', 'musicals'];
	var searchURL = "https://api.giphy.com/v1/gifs/search?q=" +
  topics + "&api_key=dc6zaTOxFJmzC&limit=10";
	var prefetchArray = []; //Array to prefetch the gifs in so there is no delay on mouseover swap
	
	var generateImageElements = (response) => {
	...
	const init = () => {
		...
		topics.reverse().forEach((topic) => createSearchItem(topic, false));
